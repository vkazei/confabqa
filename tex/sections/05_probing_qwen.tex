...
The Confabulation Atlas (Figure \ref{fig:atlas}, Appendix A.4) situates
the numbers geometrically: a \emph{supervised} 2-D projection of the
layer-18 hidden states of all 784 questions. An \emph{unsupervised}
PCA(2) control accompanies it, in which PC1+PC2 alone already support an
\(86.9\%\) refusal-vs-wrong probe (vs.~\(73.2\%\) majority), so the
refusal separation is not an artifact of supervised projection.
...
\texttt{refusal}, and 402 (51.3\%) \texttt{wrong} (breakdowns in
Appendix A, Table \ref{tbl:bycategory}). The cutoff manipulation works
as designed: pre-cutoff accuracy is 47.3\% (140/296) against 19.5\%
(95/488) post-cutoff. In the obscure-pre-cutoff cell, the benchmark's
...
disconfound needs. All 147 refusals fall in the post-cutoff cell, so
refusal is triggered by the knowledge-cutoff cue rather than intrinsic
uncertainty. This motivates the \texttt{refusal\_vs\_wrong\_within\_post}
probe of Section 4.
...
overlap in \([-0.2, -0.05]\). It is weakly informative, not a usable
...
layer 14. By layer 28 the refusals pull into a distinct cluster with no
supervision, a tight group of confident confabulations separating out on
its own. That late-layer refusal cluster is the geometry the
refusal-vs-wrong probe reads.
...
Section 8.1 bootstrap confirms the reading: the all-items margin is real,
the disconfounded within-stratum margins consistent with zero.
(2)~\emph{Cutoff is easy to predict either way}: text features alone
reach \(94.0\)--\(95.2\%\) (the year mentioned is a near-perfect
predictor), and the probe at \(98.2\%\) adds \(+3.0\) pp. This is a
manipulation check, not a finding about internals.
