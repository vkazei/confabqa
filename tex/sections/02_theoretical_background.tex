...
All three SLMs in the study are decoder-only transformers with causal attention.
The final-layer state at the last position is projected through the
LM head onto the vocabulary \(\mathcal{V}\), and
...
All generation is greedy deterministic, 
so each emitted token is the mode at that position, two metrics track confidence.
\(\bar\ell\) is the mean
log-probability of the emitted (argmax) tokens, (\(\bar\ell = -0.05\)
corresponds to \(\exp(\bar\ell)=\approx 0.95\) mean geometric probability per token). \(\bar H\) is the mean
Shannon entropy of the next-token distribution.  
Formal definitions (Eq.~\eqref{eq:lbar}), the worst-step
diagnostic \(\min_t \ell_t\), and the empirical anti-correlation of the
two are in Appendix B.6.

% ----- 2.2 The prefill hidden state -----
\subsection{2.2 The prefill hidden state}\label{the-prefill-hidden-state-as-the-canonical-going-in-representation}
...
\subsection{2.3 Linear probe and its pipeline}\label{why-a-linear-probe-and-the-specific-pipeline-i-use}
