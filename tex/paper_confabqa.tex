...
On Qwen3-1.7B, Gemma 2 2B, and Llama 3.2 3B, the probe separates upcoming refusals from confabulations beyond prompt features.
...
While the directions are over \(45^\circ\) different a push at or below the state's own norm along any of these directions flips most confabulations into refusal openings dominated by the ``As\ldots'' family.
Forcing the opener without touching the hidden state yields the same one-in-three judged-refusal rate as any push.
The opposite push collapses refusals mostly into confabulations, yet leaves originally correct answers intact, proving the usefulness of steering along found directions.
Confabulations do not resolve into a concentrated feature in a dictionary or a token signature on Qwen3-1.7B.
The dataset and evaluation design break two typical structural issues in confabulation studies.
ConfabQA is a \(784\)-item synthetic benchmark whose obscure pre-cutoff category breaks the well-known correctness/knowledge-cutoff confound.
...
Correctness signal is model-dependent: only for Llama 3.2 3B it is portable: \(81\%\) of the probe's margin over test majority survives transfer between [Confab, Pop, Trivia]QA.
...
\citep{azaria2023internal, marks2024geometry, orgad2024show, kossen2024semantic}, fitting linear
classifiers on hidden states (diagnostic classifiers; \citealp{alain2017probes}) and reporting correctness-prediction
...
\item
  ...
  content, and closing that gap is left to future work. This replicates \citeauthor{arditi2024refusal}'s (\citeyear{arditi2024refusal}) single-direction finding in the
...
substituted for the probe direction (feature-steering lineage: \citealp{templeton2024scaling}; on \(30\) originally-wrong items,
