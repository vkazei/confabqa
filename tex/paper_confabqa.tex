...
A language model \textbf{confabulates} when it produces a fluent,
confident answer that is factually wrong (the term follows \citealt{farquhar2024semantic}). \citet{kadavath2022know} argued that LLMs ``mostly know what
they know''; a probing literature followed \citep{azaria2023internal, marks2024geometry}, fitting linear classifiers on hidden states and
reporting correctness-prediction accuracies in the \(80\)--\(90\%\)
range, with behavioral detectors (sampling consistency, \citealp{manakul2023selfcheck}; semantic entropy, \citealp{farquhar2024semantic}) as the black-box
complement. Two concurrent 2026 studies push back: \citet{singh2026introspect} show
that above-chance ``introspection'' probe accuracy on small LMs is
explained by features of the prompt itself, and \citet{sahoo2026linear} find the
same for reasoning-mode probes. The question is no longer whether the
probe's numbers are high, but whether they are \emph{evidence}.

This paper is a comprehensive small-LM test of that question. I
introduce \textbf{ConfabQA}, a probing benchmark of 4 domains
...
know.'' The closest mechanistic analogue is \citet{ferrando2025entity},
...
\item
  ...
  on Qwen3 and Gemma (Sections 6.2 and 7.2). This replicates \citeauthor{arditi2024refusal}'s (\citeyear{arditi2024refusal}) single-direction finding in the epistemic-abstention
