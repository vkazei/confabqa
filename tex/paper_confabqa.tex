...
When a small language model (SLM) is asked a question whose answer it
does not hold (a fact after its training cutoff, or an obscure one it
never learned), it does one of two things: refuse, or confabulate a
fluent, confident wrong answer.
The term, argued for over ``hallucination'' by
\citet{smith2023confabulation}, names pattern-based reconstruction the
...
accuracies in the \(80\)--\(90\%\) range, with behavioral detectors
(sampling consistency, \citealp{manakul2023selfcheck}; semantic
entropy, \citealp{farquhar2024semantic}) as the black-box complement. A
...
\textbf{Relation to prior work.} The diagnostic-classifier framework is \citeauthor{alain2017probes}'s (\citeyear{alain2017probes}); tuned lenses \citep{belrose2023tunedlens} and
contrast-consistent probes \citep{burns2023discovering} apply it to large LMs.
The prompt-feature baseline used here is a \emph{selectivity control} in
the lineage of \citet{hewitt2019control}, who showed that probe accuracy
without a control task mostly measures the probe rather than the
representation (\citealt{belinkov2022probing} surveys the failure modes). Such controls remain rare in the correctness-probing literature, where time-sensitive
question sets routinely conflate ``post-cutoff'' with ``model doesn't
know.'' The closest mechanistic analogue is probably \citet{ferrando2025entity},
whose \emph{entity-recognition latents} in Gemma-2 SAEs causally steer
between answers and refusals; the present work recovers the same class
of feature via a probe + SAE pipeline on a different model family with a
disconfounded benchmark (§6.3), and adds a cross-dataset transfer test
of content-invariance (§8.3).
