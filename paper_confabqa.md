...
variance $\mathrm{EV}=0.82$ and cosine similarity $0.90$ to the original---acceptable base$\to$instruct transfer (`sae_test_reconstruction.py`,
`sae_layer_sweep.py`). Earlier layers transfer worse (EV $0.54$--$0.70$);
...
`sae_causal_ablation.py` and `figures/sae_causal_ablation.{json,md,png}`.
...
Raw test code and per-subsample values are released as `refusal_channel_test.py` and
...
python *_evaluate.py` recipes. All experiments ran on a single Apple M1 Pro
...
`figures/qwen3_1_7b/10_pca_robustness.json`, generated by `make_robustness_check.py`, which
