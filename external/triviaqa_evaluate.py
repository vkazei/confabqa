"""Phase 2 steps 3-4: generate + grade the TriviaQA sample with Qwen3-1.7B
...
and `judge` from judge.py, and writes its output to data/triviaqa_sample/...
...
Pipeline preserved exactly:
  ...
  - Same judge prompt (judge.py JUDGE_SYSTEM + JUDGE_TEMPLATE) with TriviaQA
    gold + aliases plugged in.

Caveats reported in output:
  - The judge has been calibrated on v1.0/v1.3 (Cohen kappa = 0.892 / 1.0
    against Claude); it has NOT been independently validated on TriviaQA's
    distribution. triviaqa_evaluate writes a 20-item spot-check pack to
    figures/.../triviaqa_judge_spotcheck.md so the user can eyeball it.
"""
...
def _paths_for_suffix(suffix: str):
    base = Path(f"data/triviaqa_sample{suffix}")
    return {
        "questions": base / "questions_triviaqa_n800.json",
...
def main():
    ...
    for i, q in enumerate(questions, 1):
        ...
        response = {
            ...
            "triviaqa_qid": q.get("triviaqa_qid"),
            "triviaqa_n_aliases": q.get("triviaqa_n_aliases"),
            "triviaqa_answer_word_len": q.get("triviaqa_answer_word_len"),
            "triviaqa_answer_type": q.get("triviaqa_answer_type"),
            "correct": is_correct,
            **result,
        }
        ...
        print(f"[{i}/{len(questions)}] {qid} [{mark}] "
              f"alen={q['triviaqa_answer_word_len']}  lp={result['mean_logprob']:+.3f}  "
    ...
    for f in sample:
        ...
        md.append(f"---\n\n**{r['question_id']}**  "
                  ...
                  f"answer word-len: {r.get('triviaqa_answer_word_len')})\n\n")
